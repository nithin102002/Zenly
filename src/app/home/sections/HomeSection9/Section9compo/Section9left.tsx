...
const Section9left = () => {
  ...
  const handleButtonClick = (type: keyof typeof textOptions) => {
    ...
    setSelectedButton(type as string); // Cast to string
  };

  return (
    <div className="h-[50%] w-full text-black lg:h-full lg:w-[50%] flex flex-col transition duration-300 bg-[#F3F3F3] rounded-lg">
      <div className="flex-1 w-full flex items-center justify-center mobile:h-[33.33%]">
      ...
      {/* Buttons Section */}
      <div className="flex-1 w-full flex items-center justify-center mobile:h-[33.33%]">
        <div className="h-[80%] w-full flex gap-2 items-center justify-center p-2 flex-wrap">
          {Object.keys(textOptions).map((key) => (
            <button 
              ...
              className={`text-xs font-rubik mobile:text-base sm:text-sm md:text-base lg:text-lg px-4 mobile:px-6 sm:px-6 md:px-8 lg:px-10 py-1 mobile:py-3 sm:py-2 md:py-2 lg:py-3 rounded font-semibold transition duration-300
                ${selectedButton === key ? 'bg-yellow-500 text-black' : 'border-t-4 border-yellow-500 text-gray-700'}
              `}
            >
      ...
      <div className="flex-1 w-full flex items-center justify-center mobile:h-[33.33%]">
        <div className="h-[80%] w-[80%] flex items-center justify-start p-2 transition-transform duration-300 hover:scale-105">
